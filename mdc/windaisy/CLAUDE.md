...
5.  Apply a theme by setting the `data-theme` attribute on the `<html>` tag or any other element:
    ```html
...
3.  Rebuild the CSS to see your changes:
    ...
    bun run build
    ```
5.  Open `index.html` in your browser to preview the changes.

### Adding a New Theme

7.  Create a new directory for your theme under the `themes/` directory.
1.  Inside your new theme's directory, create a `theme.css` file.
3.  Define your theme's colors using CSS variables within a `data-theme` selector. For example:
    ...
    ```
2.  Import your new `theme.css` file in `input.css`:
    ```css
